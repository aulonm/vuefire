...
export function spyUnbind(ref: FirestoreReference): SpyInstance<any, any> {
...
export function spyOnSnapshot(ref: FirestoreReference): SpyInstance<any, any> {
  //return jest.spyOn(ref, 'onSnapshot')
  const onSnapshot = ref.onSnapshot.bind(ref)
  // @ts-ignore
  return (ref.onSnapshot = vi.fn((...args) => onSnapshot(...args)))
}
...
export const createOps = (localWalkSet: WalkSet = walkSet): OperationsType => ({
  add: vi.fn((array, index, data) => array.splice(index, 0, data)),
  set: vi.fn(localWalkSet),
  remove: vi.fn((array, index) => array.splice(index, 1)),
})
